        ...
        <div class="content right">
            ...
            <div class="sbox" style="margin-top:10px">
                <div class="khungepih1">
                    <h1 class="epih1">{{ $currentMovie->name }} ({{ $currentMovie->origin_name }}) &#8211; 
                        {{ $episode->name }}</h1>
                        <div class="views"><span>{{ $currentMovie->view_total }}</span> </div>
                </div>
                <div class="box-server">
                    <div class="server-btnlink">

                @foreach ($currentMovie->episodes->sortBy([['server', 'asc']])->groupBy('server') as $server => $data)
                <div id='serie_contenido' style='padding-top: 5px'>
                    <div id='seasons'>
                        <div class='se-c'>
                            <span class="se-t se-o"><span class="server_name"> {{ $server }}</span></span>
            ...
            {{-- <div class="khunginfo">
                <div itemprop="description" class="wp-content">{{ $currentMovie->content }}</div>

            </div> --}}
            <div class="khunginfo">
                <p id="movie-content-short">{{ Str::limit($currentMovie->content, 350) }}</p>
                <p id="movie-content-full" style="display: none;">{{ $currentMovie->content }}</p>
                @if (strlen($currentMovie->content) > 350)
                <div class="btn-morecontent">
                    <div id="toggle-content">Xem Thêm</div>
                </div>
                @endif
            </div>
...
<script>
    document.getElementById('toggle-content').addEventListener('click', function() {
        var shortContent = document.getElementById('movie-content-short');
        var fullContent = document.getElementById('movie-content-full');
